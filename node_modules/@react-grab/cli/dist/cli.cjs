...
let _antfu_ni = require("@antfu/ni");
...
const detectPackageManager = async (projectRoot) => {
	const detected = await (0, _antfu_ni.detect)({ cwd: projectRoot });
	if (detected) {
...
const getXdgConfigHome = () => node_process.default.env.XDG_CONFIG_HOME || node_path.default.join(node_os.default.homedir(), ".config");
const getBaseDir = () => {
	const homeDir = node_os.default.homedir();
	if (node_process.default.platform === "win32") return node_process.default.env.APPDATA || node_path.default.join(homeDir, "AppData", "Roaming");
	if (node_process.default.platform === "darwin") return node_path.default.join(homeDir, "Library", "Application Support");
	return getXdgConfigHome();
};
const getZedConfigPath = () => {
	if (node_process.default.platform === "win32") return node_path.default.join(getBaseDir(), "Zed", "settings.json");
	return node_path.default.join(node_os.default.homedir(), ".config", "zed", "settings.json");
};
const getOpenCodeConfigPath = () => {
	const configDir = node_path.default.join(getXdgConfigHome(), "opencode");
	const jsoncPath = node_path.default.join(configDir, "opencode.jsonc");
...
const promptMcpInstall = async () => {
	const { selectedAgents } = await prompts$1({
		type: "multiselect",
		name: "selectedAgents",
...
const configure = new commander.Command().name("configure").alias("config").description("configure React Grab options").option("-y, --yes", "skip confirmation prompts", false).option("-k, --key <key>", "activation key (e.g., Meta+K, Ctrl+Shift+G, Space)").option("-m, --mode <mode>", "activation mode (toggle, hold)").option("--hold-duration <ms>", "key hold duration in milliseconds (for hold mode)").option("--allow-input <boolean>", "allow activation inside input fields (true/false)").option("--context-lines <lines>", "max context lines to include").option("--cdn <domain>", "CDN domain (e.g., unpkg.com, custom.react-grab.com)").option("-c, --cwd <cwd>", "working directory (defaults to current directory)", process.cwd()).action(async (opts) => {
	...
	try {
		...
		if (hasFlags) {
			...
			const { selectedOption } = await prompts$1({
				type: "autocomplete",
				name: "selectedOption",
			...
			if (selectedOption === "activationKey") {
				const { selectedCombo } = await prompts$1({
					type: "autocomplete",
					name: "selectedCombo",
			...
			if (selectedOption === "keyHoldDuration") {
				const { keyHoldDuration } = await prompts$1({
					type: "number",
			...
			if (selectedOption === "allowActivationInsideInput") {
				const { allowActivationInsideInput } = await prompts$1({
					type: "confirm",
			...
			if (selectedOption === "maxContextLines") {
				const { maxContextLines } = await prompts$1({
					type: "number",
		...
		if (!result.success) {
			...
			logger.break();
			console.log(`  ${picocolors.default.cyan(configJson)}`);
			logger.break();
			process.exit(1);
		}
...
const installPackages = (packages, packageManager, projectRoot, isDev = true) => {
	...
	(0, node_child_process.execSync)(fullCommand, {
		cwd: projectRoot,
		stdio: "inherit",
		env: {
			...process.env,
...
const printSubprojects = (searchRoot, sortedProjects) => {
	...
	for (const project of sortedProjects) {
		...
		const relativePath = (0, node_path.relative)(searchRoot, project.path);
...
const init = new commander.Command().name("init").alias("setup").description("initialize React Grab in your project").option("-y, --yes", "skip confirmation prompts", false).option("-f, --force", "force overwrite existing config", false).option("-k, --key <key>", "activation key (e.g., Meta+K, Ctrl+Shift+G, Space)").option("--skip-install", "skip package installation", false).option("--pkg <pkg>", "custom package URL for CLI (e.g., grab)").option("-c, --cwd <cwd>", "working directory (defaults to current directory)", process.cwd()).action(async (opts) => {
	...
	try {
		...
		if (projectInfo.hasReactGrab && !opts.force) {
			...
			logger.break();
			const { wantCustomizeOptions } = await prompts$1({
				type: "confirm",
			...
			logger.break();
			const { wantAddMcp } = await prompts$1({
				type: "confirm",
				name: "wantAddMcp",
		...
		if (projectInfo.framework === "unknown") {
			...
			if (reactProjects.length > 0) {
				...
				logger.break();
				const { selectedProject } = await prompts$1({
					type: "select",
