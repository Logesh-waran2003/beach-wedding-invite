 ...
 */
import { A as $PROXY, B as createResource, C as render, F as batch, G as on, H as createSignal, I as createComponent, K as onCleanup, L as createEffect, R as createMemo, S as memo, U as getListener, V as createRoot, a as isMac, g as isElementConnected, h as isKeyboardEventTriggeredByInput, i as isEventFromOverlay, j as $TRACK, m as hasTextSelectionOnPage, n as saveToolbarState, p as hasTextSelectionInInput, r as resolveActionEnabled, s as clampToRange, t as loadToolbarState } from "./state-BIp7lpky.js";
import { $ as MOUNT_ROOT_RECHECK_DELAY_MS, A as waitUntilNextFrame, K as FEEDBACK_DURATION_MS, M as detectCspNonce, N as getTagName, O as nativeCancelAnimationFrame, P as ARROW_KEYS, Q as MODIFIER_KEYS, R as BOX_MODEL_GAP_THRESHOLD_PX, S as getNearestComponentName, T as resolveSource, U as DRAG_SELECTION_COVERAGE_THRESHOLD, V as DEFAULT_ACTION_ID, Z as MAX_SESSION_STORAGE_SIZE_BYTES, _ as isRootElement, a as freezeGlobalAnimations, b as getElementContext, c as clearElementPositionCache, d as freezePseudoStates, dt as TOOLBAR_DEFAULT_POSITION_RATIO, et as NEXTJS_REVALIDATION_DELAY_MS, f as resumePointerEventsFreeze, g as isValidGrabbableElement, gt as Z_INDEX_OVERLAY, h as clearVisibilityCache, ht as ZOOM_DETECTION_THRESHOLD, i as freezeAnimations, j as hideFromThirdParties, k as nativeRequestAnimationFrame, l as getElementAtPosition, m as unfreezePseudoStates, mt as VERSION, n as logRecoverableError, o as unfreezeGlobalAnimations, p as suspendPointerEventsFreeze, r as freezeAllAnimations, s as openFile, t as freezeUpdates, tt as OFFSCREEN_POSITION, u as getElementsAtPoint, v as checkIsNextProject, w as getStackContext, y as getComponentDisplayName } from "./freeze-updates-DimHj2de.js";
import { n as extractElementCss, r as createElementSelector, t as disposeBaselineStyles } from "./extract-element-css-CPea0n7q.js";
...
//#endregion
//#region src/utils/create-box-model-bounds.ts
...
const parseSides = (style, property) => ({
...
const insetBounds = (bounds, inset, borderRadius) => ({
	...bounds,
...
const outsetBounds = (bounds, outset, borderRadius) => ({
	...bounds,
...
const insetCornerRadii = (radii, sides) => ({
...
const formatCornerRadii = (radii) => `${radii.topLeft}px ${radii.topRight}px ${radii.bottomRight}px ${radii.bottomLeft}px`;
const isInFlowChild = (child) => {
	const childStyle = window.getComputedStyle(child);
	return childStyle.display !== "none" && !OUT_OF_FLOW_POSITIONS.has(childStyle.position);
};
const hasVisibleSize = (rect) => rect.width > 0 || rect.height > 0;
const computeAxisGaps = (childRects, contentBounds, axis) => {
	const isColumnAxis = axis === "column";
	const sortedRects = [...childRects].sort((a, b) => isColumnAxis ? a.top - b.top : a.left - b.left);
	const gaps = [];
	for (let childIndex = 0; childIndex < sortedRects.length - 1; childIndex++) {
		...
		const gapSize = (isColumnAxis ? nextRect.top : nextRect.left) - gapStart;
		if (gapSize > .5) gaps.push(isColumnAxis ? {
			x: contentBounds.x,
			y: gapStart,
...
const computeChildGaps = (element, style, contentBounds) => {
	if (!LAYOUT_DISPLAYS.has(style.display) || element.children.length < 2) return [];
	const childRects = Array.from(element.children).filter(isInFlowChild).map((child) => child.getBoundingClientRect()).filter(hasVisibleSize);
	if (childRects.length < 2) return [];
	if (GRID_DISPLAYS.has(style.display)) return [...computeAxisGaps(childRects, contentBounds, "row"), ...computeAxisGaps(childRects, contentBounds, "column")];
	return computeAxisGaps(childRects, contentBounds, style.flexDirection === "column" || style.flexDirection === "column-reverse" ? "column" : "row");
};
const createBoxModelBounds = (element) => {
	...
	const paddingRadii = insetCornerRadii(parseCornerRadii(style), borderSides);
	...
	return {
		...
		padding,
		content,
		gaps: computeChildGaps(element, style, content)
	};
};
...
const init = (rawOptions) => {
	...
	return createRoot((dispose) => {
		...
		const inspectBoxModel = createMemo(() => {
			...
			const element = activeIdx >= 0 && activeIdx < ancestors.length ? ancestors[activeIdx] : effectiveElement();
			if (!element) return void 0;
			store.viewportVersion;
		...
		if (pluginRegistry.store.theme.enabled) import("./renderer-B_n0Qkj8.js").then(({ ReactGrabRenderer }) => {
			if (disposed) return;
