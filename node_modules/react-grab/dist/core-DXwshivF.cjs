 ...
 */
const require_state = require('./state-CNwHl7x9.cjs');
const require_freeze_updates = require('./freeze-updates-CHz5g9ty.cjs');
const require_extract_element_css = require('./extract-element-css-Byw77np5.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-D-KPGFz4.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
