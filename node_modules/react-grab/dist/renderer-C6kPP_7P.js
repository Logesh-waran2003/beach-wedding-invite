...
const OverlayCanvas = (props) => {
	...
	const buildRingPath = (outer, inner) => {
		const path = new Path2D();
		const { x: ox, y: oy, width: ow, height: oh } = outer.current;
		if (ow > 0 && oh > 0) {
			const outerRadius = Math.min(outer.borderRadius, ow / 2, oh / 2);
			if (outerRadius > 0) path.roundRect(ox, oy, ow, oh, outerRadius);
			else path.rect(ox, oy, ow, oh);
		}
		const { x: ix, y: iy, width: iw, height: ih } = inner.current;
		if (iw > 0 && ih > 0) {
			const innerRadius = Math.min(inner.borderRadius, iw / 2, ih / 2);
			if (innerRadius > 0) path.roundRect(ix, iy, iw, ih, innerRadius);
			else path.rect(ix, iy, iw, ih);
		}
		return path;
	};
