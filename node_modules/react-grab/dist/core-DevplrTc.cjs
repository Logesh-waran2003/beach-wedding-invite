 ...
 */
const require_state = require('./state-DgXCGsV0.cjs');
const require_freeze_updates = require('./freeze-updates-DBrTX88l.cjs');
const require_extract_element_css = require('./extract-element-css-CyXOJgb7.cjs');
	...
	return require_state.createRoot((dispose) => {
		...
		require_state.createEffect(require_state.on(isActivated, (activated, previousActivated) => {
			if (activated && !previousActivated) {
				...
				require_freeze_updates.buildSpatialIndex();
				document.body.style.touchAction = "none";
				unlockViewportZoom = lockViewportZoom();
			} else if (!activated && previousActivated) {
				require_freeze_updates.destroySpatialIndex();
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BaKHVtNX.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
