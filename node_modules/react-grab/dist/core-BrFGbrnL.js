...
const computeAxisGaps = (childRects, contentBounds, axis) => {
	const sortedRects = [...childRects].sort((a, b) => axis === "column" ? a.top - b.top : a.left - b.left);
	const gaps = [];
	for (let childIndex = 0; childIndex < sortedRects.length - 1; childIndex++) {
		const gapStart = axis === "column" ? sortedRects[childIndex].bottom : sortedRects[childIndex].right;
		const gapSize = (axis === "column" ? sortedRects[childIndex + 1].top : sortedRects[childIndex + 1].left) - gapStart;
