 ...
 */
const require_state = require('./state-Cz3Edzco.cjs');
const require_freeze_updates = require('./freeze-updates-D2G--pol.cjs');
const require_extract_element_css = require('./extract-element-css-Dl3NdNM3.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-HA4Q-6DR.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
