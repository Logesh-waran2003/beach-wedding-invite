 ...
 */
const require_state = require('./state-GYNj-s4j.cjs');
const require_freeze_updates = require('./freeze-updates-NZBhGPlZ.cjs');
const require_extract_element_css = require('./extract-element-css-BBCKagGd.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-KELVQhe3.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
