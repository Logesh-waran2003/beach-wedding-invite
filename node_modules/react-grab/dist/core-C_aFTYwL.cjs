 ...
 */
const require_state = require('./state-CHkH1HWR.cjs');
const require_freeze_updates = require('./freeze-updates-Dhn0vQ2A.cjs');
const require_extract_element_css = require('./extract-element-css-BVEPCg6d.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CTMaw1TM.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
