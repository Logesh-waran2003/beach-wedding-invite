 ...
 */
const require_state = require('./state-BMcnzWRu.cjs');
const require_freeze_updates = require('./freeze-updates-9DzOH4Jf.cjs');
const require_extract_element_css = require('./extract-element-css-DkcvKzi6.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-eu55Cb4q.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
