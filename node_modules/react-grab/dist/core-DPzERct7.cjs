 ...
 */
const require_state = require('./state-CG3t-Yn-.cjs');
const require_freeze_updates = require('./freeze-updates-DiSujId0.cjs');
const require_extract_element_css = require('./extract-element-css-BNwLombi.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-UzGAOs5Y.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
