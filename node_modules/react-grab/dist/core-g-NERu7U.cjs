 ...
 */
const require_state = require('./state-B7sdnwvW.cjs');
const require_freeze_updates = require('./freeze-updates-DiS5CmhW.cjs');
const require_extract_element_css = require('./extract-element-css-7EemA_HC.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-C5XAhpnj.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
