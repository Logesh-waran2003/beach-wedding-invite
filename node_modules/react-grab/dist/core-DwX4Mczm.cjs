 ...
 */
const require_state = require('./state-QUt2Guhk.cjs');
const require_freeze_updates = require('./freeze-updates-DWqZclQ3.cjs');
const require_extract_element_css = require('./extract-element-css-CEF2jaM9.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DV0gpYDG.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
