 ...
 */
const require_state = require('./state-LDfWgKVc.cjs');
const require_freeze_updates = require('./freeze-updates-f9IVBHUv.cjs');
const require_extract_element_css = require('./extract-element-css-e3IvwA1l.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-7ltK7cdB.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
