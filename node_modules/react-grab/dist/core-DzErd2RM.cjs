 ...
 */
const require_state = require('./state-DRLIeaAT.cjs');
const require_freeze_updates = require('./freeze-updates-BOQ-cieC.cjs');
const require_extract_element_css = require('./extract-element-css-D9cUKyM5.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BtOSJgs1.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
