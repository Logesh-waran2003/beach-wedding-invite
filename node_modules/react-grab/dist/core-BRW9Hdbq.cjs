 ...
 */
const require_state = require('./state-B0HhqxQx.cjs');
const require_freeze_updates = require('./freeze-updates-Bbj-_V7_.cjs');
const require_extract_element_css = require('./extract-element-css-DbMxeVUO.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CbB7twVg.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
