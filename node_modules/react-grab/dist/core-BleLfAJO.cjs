 ...
 */
const require_state = require('./state-CAlz_Smp.cjs');
const require_freeze_updates = require('./freeze-updates-ebq-9RLD.cjs');
const require_extract_element_css = require('./extract-element-css-5JM91YvS.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-p8vR_Kj7.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
