 ...
 */
const require_state = require('./state-D4Y_ncIU.cjs');
const require_freeze_updates = require('./freeze-updates-Cooy9ZoE.cjs');
const require_extract_element_css = require('./extract-element-css-BS8lFERn.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-s84QsbhG.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
