 ...
 */
const require_state = require('./state-B5Rz3qze.cjs');
const require_freeze_updates = require('./freeze-updates-Bh43-VHz.cjs');
const require_extract_element_css = require('./extract-element-css-AAVdHD1T.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DVdySK-D.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
