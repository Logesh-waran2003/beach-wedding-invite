...
const ELEMENT_AT_POINT_INDEX_ROOT_MARGIN_PX = 1e4;
...
//#endregion
//#region src/utils/is-decorative-overlay.ts
...
const isDecorativeOverlay = (element, computedPosition) => (computedPosition === "absolute" || computedPosition === "sticky") && !REPLACED_TAGS.has(element.tagName) && element.childElementCount === 0 && (element.textContent?.trim().length ?? 0) === 0;
...
//#endregion
//#region src/utils/hilbert-r-tree.ts
const NODE_SIZE = 16;
const upperBound = (value, levelBounds) => {
	let low = 0;
	let high = levelBounds.length - 1;
	while (low < high) {
		const mid = low + high >> 1;
		if (levelBounds[mid] > value) high = mid;
		else low = mid + 1;
	}
	return levelBounds[low];
};
const swapItems = (values, boxes, indices, indexA, indexB) => {
...
const sortByHilbert = (values, boxes, indices, left, right) => {
	...
	while (stack.length) {
		...
		const pivot = valueLeft > valueMid !== valueLeft > valueRight ? valueLeft : valueMid < valueLeft !== valueMid < valueRight ? valueMid : valueRight;
...
const hilbert = (coordX, coordY) => {
	let xorXY = coordX ^ coordY;
	let invertedXor = 65535 ^ xorXY;
	let invertedOr = 65535 ^ (coordX | coordY);
	let maskedAnd = coordX & (coordY ^ 65535);
	...
	let interleaved1 = invertedXor | 65535 ^ (interleaved0 | xorXY);
	interleaved0 = (interleaved0 | interleaved0 << 8) & 16711935;
	interleaved0 = (interleaved0 | interleaved0 << 4) & 252645135;
	interleaved0 = (interleaved0 | interleaved0 << 2) & 858993459;
	interleaved0 = (interleaved0 | interleaved0 << 1) & 1431655765;
	interleaved1 = (interleaved1 | interleaved1 << 8) & 16711935;
	interleaved1 = (interleaved1 | interleaved1 << 4) & 252645135;
	interleaved1 = (interleaved1 | interleaved1 << 2) & 858993459;
	interleaved1 = (interleaved1 | interleaved1 << 1) & 1431655765;
	return (interleaved1 << 1 | interleaved0) >>> 0;
};
var HilbertRTree = class {
	...
	finish() {
		if (this.position >> 2 !== this.numItems) throw new Error(`Added ${this.position >> 2} items when expected ${this.numItems}.`);
		...
		for (let itemIndex = 0, boxPosition = 0; itemIndex < this.numItems; itemIndex++) {
			...
			hilbertValues[itemIndex] = hilbert(Math.floor(hilbertMax * ((rectMinX + rectMaxX) / 2 - this.minX) / width), Math.floor(hilbertMax * ((rectMinY + rectMaxY) / 2 - this.minY) / height));
		}
	...
	search(minX, minY, maxX, maxY) {
		if (this.position !== this.boxes.length) throw new Error("Data not yet indexed - call finish().");
		...
		while (nodeIndex !== void 0) {
			...
			for (let boxPosition = nodeIndex; boxPosition < nodeEnd; boxPosition += 4) {
				if (maxX < this.boxes[boxPosition]) continue;
				if (maxY < this.boxes[boxPosition + 1]) continue;
				if (minX > this.boxes[boxPosition + 2]) continue;
				if (minY > this.boxes[boxPosition + 3]) continue;
				const candidateIndex = this.indices[boxPosition >> 2] | 0;
				if (nodeIndex >= this.numItems * 4) searchQueue.push(candidateIndex);
				else results.push(candidateIndex);
			}
...
//#endregion
//#region src/utils/compare-stacking-order.ts
const STACKING_PROPS = /\b(?:position|z-index|opacity|transform|mix-blend-mode|filter|isolation|clip-path|backdrop-filter|perspective|contain)\b/;
const getParentElement = (node) => {
	const parentNode = node.parentNode;
	return (parentNode && "host" in parentNode ? parentNode.host : null) ?? node.parentElement;
};
...
const hasPaintContainment$1 = (containValue) => {
	if (containValue === "none" || containValue === "size" || containValue === "layout") return false;
	return containValue === "paint" || containValue === "strict" || containValue === "content" || containValue.includes("paint");
};
const isStackingContext = (node, style) => {
	...
	if (hasPaintContainment$1(style.contain)) return true;
...
const getNearestStackingContextZIndex = (ancestors) => {
	let ancestorIndex = ancestors.length;
	while (ancestorIndex--) {
		const style = getComputedStyle(ancestors[ancestorIndex]);
		if (isStackingContext(ancestors[ancestorIndex], style)) return Number(style.zIndex) || 0;
	}
	return 0;
};
const compareStackingOrder = (elementA, elementB) => {
...
//#endregion
//#region src/utils/element-at-point-index.ts
...
const buildElementAtPointIndex = () => {
	destroyElementAtPointIndex();
	let didObserveAnyElement = false;
	const accumulatedElements = [];
	const accumulatedRects = [];
	const accumulatedFixedElements = [];
	const observer = new IntersectionObserver((entries) => {
		...
		for (const entry of entries) {
			...
			if (getComputedStyle(targetElement).position === "fixed") {
				const computedZIndex = getComputedStyle(targetElement).zIndex;
		...
		if (accumulatedElements.length > 0) {
			...
			for (const rect of accumulatedRects) tree.add(rect.left, rect.top, rect.right, rect.bottom);
			tree.finish();
		}
	...
	const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_ELEMENT, { acceptNode: (node) => SKIP_TAGS.has(node.tagName) ? NodeFilter.FILTER_REJECT : NodeFilter.FILTER_ACCEPT });
...
const PAINT_CONTAIN_VALUES = new Set([
	"paint",
	"strict",
	"content"
]);
const CLIPS_X = 1;
const CLIPS_Y = 2;
let clipStateCache = /* @__PURE__ */ new WeakMap();
const hasPaintContainment = (containValue) => {
	for (const keyword of containValue.split(" ")) if (PAINT_CONTAIN_VALUES.has(keyword)) return true;
	return false;
};
const getClipState = (ancestor) => {
...
const isVisibleAtPoint = (element, clientX, clientY) => {
	let ancestor = element.parentElement;
	while (ancestor && ancestor !== document.documentElement) {
		const clipState = getClipState(ancestor);
		if (clipState !== 0) {
			const ancestorRect = ancestor.getBoundingClientRect();
			if ((clipState & CLIPS_X) !== 0 && (clientX < ancestorRect.left || clientX > ancestorRect.right)) return false;
			if ((clipState & CLIPS_Y) !== 0 && (clientY < ancestorRect.top || clientY > ancestorRect.bottom)) return false;
		}
		ancestor = ancestor.parentElement;
	}
	return true;
};
const findTopmostFixedElement = (fixedElements, clientX, clientY) => {
	let topmost = null;
	for (const entry of fixedElements) {
		if (!entry.element.isConnected) continue;
		const rect = entry.viewportRect;
		if (clientX >= rect.left && clientX <= rect.right && clientY >= rect.top && clientY <= rect.bottom) topmost = entry;
	}
	return topmost?.element ?? null;
};
const queryElementAtPointIndex = (clientX, clientY) => {
...
const isElementAtPointIndexReady = () => currentIndex !== null;
const destroyElementAtPointIndex = () => {
	if (pendingObserver) {
	...
	clipStateCache = /* @__PURE__ */ new WeakMap();
};
...
const getElementAtPosition = (clientX, clientY) => {
	...
	if (topElement && isValidGrabbableElement(topElement)) result = topElement;
		...
		if (spatialResult && isValidGrabbableElement(spatialResult)) result = spatialResult;
	}
	if (!result) {
...
//#endregion
export { OFFSCREEN_POSITION as $, Z$1 as A, DRAG_LERP_FACTOR as B, getHTMLPreview as C, resolveSource as D, getStackContext as E, detectCspNonce as F, FROZEN_GLOW_COLOR as G, DROPDOWN_EDGE_TRANSFORM_ORIGIN as H, getTagName as I, MAX_SELECTOR_COMBINATIONS as J, HINT_FLIP_IN_ANIMATION as K, ARROW_KEYS as L, nativeRequestAnimationFrame as M, waitUntilNextFrame as N, stripInternalAttributes as O, hideFromThirdParties as P, NEXTJS_REVALIDATION_DELAY_MS as Q, ARROW_MAX_LABEL_WIDTH_RATIO as R, getElementContext as S, getStack as T, DROPDOWN_OFFSCREEN_POSITION as U, DRAG_SELECTION_COVERAGE_THRESHOLD as V, FEEDBACK_DURATION_MS as W, MODIFIER_KEYS as X, MAX_SESSION_STORAGE_SIZE_BYTES as Y, MOUNT_ROOT_RECHECK_DELAY_MS as Z, clearVisibilityCache as _, Z_INDEX_OVERLAY_CANVAS as _t, freezeGlobalAnimations as a, OVERLAY_FILL_COLOR_DRAG as at, checkIsNextProject as b, clearElementPositionCache as c, SELECTION_HINT_CYCLE_INTERVAL_MS as ct, buildElementAtPointIndex as d, TOOLBAR_DEFAULT_POSITION_RATIO as dt, OPACITY_CONVERGENCE_THRESHOLD as et, destroyElementAtPointIndex as f, TOOLBAR_SHAKE_TOOLTIP_DURATION_MS as ft, unfreezePseudoStates as g, Z_INDEX_OVERLAY as gt, suspendPointerEventsFreeze as h, ZOOM_DETECTION_THRESHOLD as ht, freezeAnimations as i, OVERLAY_FILL_COLOR_DEFAULT as it, nativeCancelAnimationFrame as j, Te$1 as k, getElementAtPosition as l, SELECTION_LABEL_OFFSCREEN_PX as lt, resumePointerEventsFreeze as m, VERSION as mt, logRecoverableError as n, OVERLAY_BORDER_COLOR_DRAG as nt, unfreezeGlobalAnimations as o, OVERLAY_FILL_COLOR_INSPECT as ot, freezePseudoStates as p, TOOLTIP_BASE_CLASS as pt, LERP_CONVERGENCE_THRESHOLD_PX as q, freezeAllAnimations as r, OVERLAY_BORDER_COLOR_INSPECT as rt, openFile as s, RELEVANT_CSS_PROPERTIES as st, freezeUpdates as t, OVERLAY_BORDER_COLOR_DEFAULT as tt, getElementsAtPoint as u, SELECTION_LERP_FACTOR as ut, isValidGrabbableElement as v, supportsDisplayP3 as vt, getNearestComponentName as w, getComponentDisplayName as x, isRootElement as y, DEFAULT_ACTION_ID as z };