 ...
 */
const require_state = require('./state-CvUeFIYo.cjs');
const require_freeze_updates = require('./freeze-updates-D_FDfZ4W.cjs');
const require_extract_element_css = require('./extract-element-css-CSQwLjs1.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-pTEQlday.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
