 ...
 */
const require_state = require('./state-BONrmZFI.cjs');
const require_freeze_updates = require('./freeze-updates-CzNwPFFT.cjs');
const require_extract_element_css = require('./extract-element-css-DgYVzCEp.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CFBRMV7E.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
