 ...
 */
const require_state = require('./state-CvL_v1tI.cjs');
const require_freeze_updates = require('./freeze-updates-Df_Uvu6i.cjs');
const require_extract_element_css = require('./extract-element-css-BCPM5VOg.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CBFkHal1.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
