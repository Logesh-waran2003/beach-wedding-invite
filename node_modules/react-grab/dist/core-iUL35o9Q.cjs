 ...
 */
const require_state = require('./state-C6mXfeV5.cjs');
const require_freeze_updates = require('./freeze-updates-N5gYvkKS.cjs');
const require_extract_element_css = require('./extract-element-css-XbuK-mSd.cjs');
	...
	return require_state.createRoot((dispose) => {
		...
		require_state.createEffect(require_state.on(isActivated, (activated, previousActivated) => {
			if (activated && !previousActivated) {
				...
				setTimeout(require_freeze_updates.buildPrehitIndex);
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-B2djGWqf.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
