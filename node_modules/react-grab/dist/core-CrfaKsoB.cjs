 ...
 */
const require_state = require('./state-B9H9jhE-.cjs');
const require_freeze_updates = require('./freeze-updates-nEPGwkwM.cjs');
const require_extract_element_css = require('./extract-element-css-CObUqX8o.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-ij6Raif8.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
