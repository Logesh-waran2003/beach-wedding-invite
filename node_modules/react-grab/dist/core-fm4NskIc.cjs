 ...
 */
const require_state = require('./state-CPFnzlGU.cjs');
const require_freeze_updates = require('./freeze-updates-B8Dg73CM.cjs');
const require_extract_element_css = require('./extract-element-css-BFHMY4eV.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DYkqJBf_.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
