 ...
 */
const require_state = require('./state-KiwTQfDP.cjs');
const require_freeze_updates = require('./freeze-updates-DZIXw9Tj.cjs');
const require_extract_element_css = require('./extract-element-css-BqgOs97z.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-D_7u_hJ3.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
