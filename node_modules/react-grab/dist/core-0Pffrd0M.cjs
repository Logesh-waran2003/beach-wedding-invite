 ...
 */
const require_state = require('./state-BNk4jRTz.cjs');
const require_freeze_updates = require('./freeze-updates-BvgLXkV4.cjs');
const require_extract_element_css = require('./extract-element-css-XrL64KIE.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BxWB8yFD.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
