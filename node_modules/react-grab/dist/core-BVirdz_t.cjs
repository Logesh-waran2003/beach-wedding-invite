 ...
 */
const require_state = require('./state-WmFuxwMh.cjs');
const require_freeze_updates = require('./freeze-updates-CWRjbYKC.cjs');
const require_extract_element_css = require('./extract-element-css-CuRxDMul.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DbZoJ-7U.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
