 ...
 */
const require_state = require('./state-DWiYbj6U.cjs');
const require_freeze_updates = require('./freeze-updates-DveYOvgK.cjs');
const require_extract_element_css = require('./extract-element-css-xqMGvriQ.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DfEHlskt.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
