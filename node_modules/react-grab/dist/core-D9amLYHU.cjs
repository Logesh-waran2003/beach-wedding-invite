...
const require_extract_element_css = require('./extract-element-css-1YIWKwtR.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		const inspectBoxModel = require_state.createMemo(() => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-q638lwQ2.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
