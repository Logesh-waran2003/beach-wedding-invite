 ...
 */
const require_state = require('./state-D1wkURAA.cjs');
const require_freeze_updates = require('./freeze-updates-OSdXjAge.cjs');
const require_extract_element_css = require('./extract-element-css-D0DD10k1.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-D6aN7FVQ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
