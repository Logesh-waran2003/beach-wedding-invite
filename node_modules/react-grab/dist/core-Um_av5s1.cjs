 ...
 */
const require_state = require('./state-YgFXzTGZ.cjs');
const require_freeze_updates = require('./freeze-updates-D_iYY8fG.cjs');
const require_extract_element_css = require('./extract-element-css-BlI6bBkh.cjs');
	...
	return require_state.createRoot((dispose) => {
		...
		require_state.createEffect(require_state.on(isActivated, (activated, previousActivated) => {
			if (activated && !previousActivated) {
				...
				require_freeze_updates.nativeRequestAnimationFrame(() => require_freeze_updates.buildPrehitIndex());
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-nOd0Tpp_.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
