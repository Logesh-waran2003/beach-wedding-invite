 ...
 */
const require_state = require('./state-C14rLkjU.cjs');
const require_freeze_updates = require('./freeze-updates-rROWmdrb.cjs');
const require_extract_element_css = require('./extract-element-css-Wtyd9fDc.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-D96AgYVa.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
