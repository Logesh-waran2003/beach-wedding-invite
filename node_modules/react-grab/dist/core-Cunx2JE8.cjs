 ...
 */
const require_state = require('./state-fUgl3MMW.cjs');
const require_freeze_updates = require('./freeze-updates-Bap53GTj.cjs');
const require_extract_element_css = require('./extract-element-css-m7aNc9-w.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Du_rV8md.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
