 ...
 */
const require_state = require('./state-B5H1ZoOT.cjs');
const require_freeze_updates = require('./freeze-updates-CDAVfEVZ.cjs');
const require_extract_element_css = require('./extract-element-css-B9IxGKsQ.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-B4KRGNUN.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
