 ...
 */
const require_state = require('./state-vqTFntIZ.cjs');
const require_freeze_updates = require('./freeze-updates-DIOR2JfB.cjs');
const require_extract_element_css = require('./extract-element-css-CjjYfqwh.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-fcJTpLyv.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
