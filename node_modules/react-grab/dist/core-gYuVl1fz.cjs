 ...
 */
const require_state = require('./state-BQzPdpzj.cjs');
const require_freeze_updates = require('./freeze-updates-f5wUOt7Q.cjs');
const require_extract_element_css = require('./extract-element-css-bqTQFGzh.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-B1kshSoh.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
