...
const isDecorativeOverlay$1 = (element, computedPosition) => (computedPosition === "absolute" || computedPosition === "sticky") && !REPLACED_TAGS.has(element.tagName) && element.childElementCount === 0 && (element.textContent?.trim().length ?? 0) === 0;
...
const isValidGrabbableElement = (element) => {
	...
	if (isDecorativeOverlay$1(element, computedStyle.position)) {
		visibilityCache.set(element, {
			isVisible: false,
			timestamp: now
		});
