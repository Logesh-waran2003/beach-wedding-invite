 ...
 */
const require_state = require('./state-Bu2yLzRa.cjs');
const require_freeze_updates = require('./freeze-updates-BwaQpKd1.cjs');
const require_extract_element_css = require('./extract-element-css-DZuEfOyc.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Cl537hJV.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
