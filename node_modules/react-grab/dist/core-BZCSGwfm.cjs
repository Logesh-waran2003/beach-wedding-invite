 ...
 */
const require_state = require('./state-qqRi72e2.cjs');
const require_freeze_updates = require('./freeze-updates-C6QCIxgV.cjs');
const require_extract_element_css = require('./extract-element-css-D5-t44PA.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-B4VrZfvu.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
