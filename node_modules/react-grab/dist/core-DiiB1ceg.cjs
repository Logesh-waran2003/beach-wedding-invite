 ...
 */
const require_state = require('./state-DIIOZJ5L.cjs');
const require_freeze_updates = require('./freeze-updates-vckQ5g3R.cjs');
const require_extract_element_css = require('./extract-element-css-D7uKLomq.cjs');
	...
	contextMenuAction: (api) => ({
		...
		onAction: async (context) => {
			await context.performWithFeedback(async () => {
				const combinedHtml = context.elements.map((element) => require_freeze_updates.stripInternalAttributes(element.outerHTML)).join("\n\n");
...
const buildElementAtPointIndex = () => {
	...
	const observer = new IntersectionObserver((entries) => {
		...
		for (const entry of entries) {
			...
			if (!require_freeze_updates.isValidGrabbableElement(targetElement)) continue;
	...
	}, { rootMargin: `${require_freeze_updates.ELEMENT_AT_POINT_INDEX_ROOT_MARGIN_PX}px` });
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CHFMrAwQ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
