 ...
 */
const require_state = require('./state-D9N4ms_q.cjs');
const require_freeze_updates = require('./freeze-updates-7iG-Wh3F.cjs');
const require_extract_element_css = require('./extract-element-css-DFC6z4WI.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DXXhDJ0U.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
