 ...
 */
const require_state = require('./state-C3amsRhX.cjs');
const require_freeze_updates = require('./freeze-updates-BDahWMVI.cjs');
const require_extract_element_css = require('./extract-element-css-DDExTxaK.cjs');
...
const buildPrehitIndex = () => {
	...
	const observer = new IntersectionObserver((entries) => {
		...
		for (const entry of entries) {
			...
			if (!require_freeze_updates.isValidGrabbableElement(entry.target)) continue;
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		const isDragging = require_state.createMemo(() => store.current.state === "active" && (store.current.phase === "dragging-select" || store.current.phase === "dragging-reposition"));
		const isDragRepositioning = require_state.createMemo(() => store.current.state === "active" && store.current.phase === "dragging-reposition");
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BuM7KYJ5.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
