 ...
 */
const require_state = require('./state-CobGqbaK.cjs');
const require_freeze_updates = require('./freeze-updates-CHLSLMwl.cjs');
const require_extract_element_css = require('./extract-element-css-TMq5QlFZ.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DZKCK4xd.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
