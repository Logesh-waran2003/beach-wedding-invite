 ...
 */
const require_state = require('./state-Dh86SJK6.cjs');
const require_freeze_updates = require('./freeze-updates-BnL7ODHj.cjs');
const require_extract_element_css = require('./extract-element-css-Be_TQN8x.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-QWqVVfrk.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
