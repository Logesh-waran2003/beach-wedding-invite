 ...
 */
const require_state = require('./state-CWVAA4gx.cjs');
const require_freeze_updates = require('./freeze-updates-OPE4GWZI.cjs');
const require_extract_element_css = require('./extract-element-css-6__8JIsQ.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-y5bbvmpY.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
