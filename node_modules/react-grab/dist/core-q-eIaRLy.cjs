 ...
 */
const require_state = require('./state-CSOk0Nl7.cjs');
const require_freeze_updates = require('./freeze-updates-C5SkX0dv.cjs');
const require_extract_element_css = require('./extract-element-css-CZFD29Fv.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CxdwO4RG.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
