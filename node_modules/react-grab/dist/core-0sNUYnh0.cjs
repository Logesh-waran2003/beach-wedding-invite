 ...
 */
const require_state = require('./state-CmHQ8YW0.cjs');
const require_freeze_updates = require('./freeze-updates-BBTslWlm.cjs');
const require_extract_element_css = require('./extract-element-css-Dic1P_Pr.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-D4HC3P0P.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
