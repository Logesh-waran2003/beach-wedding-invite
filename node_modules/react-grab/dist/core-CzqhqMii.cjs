 ...
 */
const require_state = require('./state-CVKwzwA9.cjs');
const require_freeze_updates = require('./freeze-updates-hvM561Wv.cjs');
const require_extract_element_css = require('./extract-element-css-BECI3xjA.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DDsMBuKJ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
