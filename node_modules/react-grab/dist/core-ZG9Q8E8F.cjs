 ...
 */
const require_state = require('./state-DO53YuD0.cjs');
const require_freeze_updates = require('./freeze-updates-zEU7ZI36.cjs');
const require_extract_element_css = require('./extract-element-css-BfNuYxtB.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BGkMs1zS.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
