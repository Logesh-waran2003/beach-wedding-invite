 ...
 */
const require_state = require('./state-CC-5Rf5y.cjs');
const require_freeze_updates = require('./freeze-updates-F76sDKQY.cjs');
const require_extract_element_css = require('./extract-element-css-CYrUx3sl.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DazaQ_uM.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
