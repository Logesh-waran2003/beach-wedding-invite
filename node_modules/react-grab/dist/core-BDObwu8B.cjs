 ...
 */
const require_state = require('./state-rd69-Ahy.cjs');
const require_freeze_updates = require('./freeze-updates-DTJk-XSz.cjs');
const require_extract_element_css = require('./extract-element-css-i4kYku53.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-vEbFU83P.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
