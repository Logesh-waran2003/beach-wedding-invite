 ...
 */
const require_state = require('./state-BsFg8YGu.cjs');
const require_freeze_updates = require('./freeze-updates-CosP7hI0.cjs');
const require_extract_element_css = require('./extract-element-css-DFm6jmeM.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DwNB1s_0.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
