...
const isFullViewportOverlay = (element, computedStyle) => {
	...
	if (!(element.clientWidth / window.innerWidth >= .9 && element.clientHeight / window.innerHeight >= .9)) return false;
...
const findNearestStackingContext = (ancestors) => {
	let ancestorIndex = ancestors.length;
	while (ancestorIndex--) if (createsStackingContext(ancestors[ancestorIndex])) return ancestors[ancestorIndex];
	return null;
};
const getZIndex = (node) => node && Number(getComputedStyle(node).zIndex) || 0;
...
const buildElementAtPointIndex = () => {
	...
	const observer = new IntersectionObserver((entries) => {
		...
		const tree = new HilbertRTree(Math.max(accumulatedElements.length, 1));
