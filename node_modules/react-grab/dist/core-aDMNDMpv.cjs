 ...
 */
const require_state = require('./state-DrQ9gSSC.cjs');
const require_freeze_updates = require('./freeze-updates-DoDDI_rg.cjs');
const require_extract_element_css = require('./extract-element-css-BuZ4Gnbv.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CS6SjrPh.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
