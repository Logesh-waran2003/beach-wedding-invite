 ...
 */
const require_state = require('./state-v_7H8nLM.cjs');
const require_freeze_updates = require('./freeze-updates-BLg0p8O6.cjs');
const require_extract_element_css = require('./extract-element-css-DAyyN40V.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DEekTdKp.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
