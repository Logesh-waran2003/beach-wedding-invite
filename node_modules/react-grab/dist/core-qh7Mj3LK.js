...
const computeChildGaps = (element, style, contentBounds) => {
	...
	const isColumn = FLEX_DISPLAYS.has(style.display) && (style.flexDirection === "column" || style.flexDirection === "column-reverse");
	...
	for (let childIndex = 0; childIndex < childRects.length - 1; childIndex++) {
		...
		const gapSize = (isColumn ? nextRect.top : nextRect.left) - gapStart;
		if (gapSize > .5) gaps.push(isColumn ? {
			x: contentBounds.x,
			y: gapStart,
