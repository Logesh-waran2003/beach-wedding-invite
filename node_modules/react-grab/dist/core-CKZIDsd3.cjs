 ...
 */
const require_state = require('./state-C-55iPu3.cjs');
const require_freeze_updates = require('./freeze-updates-BGnKs_Sx.cjs');
const require_extract_element_css = require('./extract-element-css-yHipz2xT.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-B6A8xzwW.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
