...
const computeAxisGaps = (childRects, contentBounds, axis) => {
	const leadingEdge = axis === "column" ? "top" : "left";
	const trailingEdge = axis === "column" ? "bottom" : "right";
	const sortedRects = [...childRects].sort((a, b) => a[leadingEdge] - b[leadingEdge]);
	const gaps = [];
	for (let childIndex = 0; childIndex < sortedRects.length - 1; childIndex++) {
		const gapStart = sortedRects[childIndex][trailingEdge];
		const gapSize = sortedRects[childIndex + 1][leadingEdge] - gapStart;
		if (gapSize > .5) gaps.push(axis === "column" ? {
			x: contentBounds.x,
			y: gapStart,
...
const computeChildGaps = (element, style, contentBounds) => {
	...
	for (const child of element.children) {
		...
		if (childStyle.display === "none" || OUT_OF_FLOW_POSITIONS.has(childStyle.position)) continue;
