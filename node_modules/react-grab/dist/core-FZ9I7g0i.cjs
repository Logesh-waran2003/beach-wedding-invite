 ...
 */
const require_state = require('./state-CVWC2k5G.cjs');
const require_freeze_updates = require('./freeze-updates-BVbTaj6z.cjs');
const require_extract_element_css = require('./extract-element-css-1OA_QWdd.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DnN6zt58.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
