 ...
 */
const require_state = require('./state-BFsVzPbx.cjs');
const require_freeze_updates = require('./freeze-updates-4OJyfefb.cjs');
const require_extract_element_css = require('./extract-element-css-DP5vfHMk.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Cr4yVgXY.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
