 ...
 */
const require_state = require('./state-CpAqZeOJ.cjs');
const require_freeze_updates = require('./freeze-updates-BmlF9pyJ.cjs');
const require_extract_element_css = require('./extract-element-css-DivWbjIv.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-WJ0b7afe.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
