 ...
 */
const require_state = require('./state-BaYNWDln.cjs');
const require_freeze_updates = require('./freeze-updates-CM5fDK1A.cjs');
const require_extract_element_css = require('./extract-element-css-BlJ328Et.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-ByOV0kvz.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
