 ...
 */
const require_state = require('./state-Bb8IYLwO.cjs');
const require_freeze_updates = require('./freeze-updates-CWywrWqT.cjs');
const require_extract_element_css = require('./extract-element-css-Bs18mwbZ.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BMLJp0gO.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
