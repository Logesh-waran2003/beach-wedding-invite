 ...
 */
const require_state = require('./state-DFSzRyLx.cjs');
const require_freeze_updates = require('./freeze-updates-oEHZiAFl.cjs');
const require_extract_element_css = require('./extract-element-css-BGMTMNoI.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Dy0-5HgM.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
