...
//#endregion
//#region src/vendor/hilbert-r-tree.ts
const NODE_SIZE = 16;
...
const clipsAtPoint = (style, rect, clientX, clientY) => {
...
const isVisibleAtPoint = (element, clientX, clientY) => {
	...
	while (ancestor && ancestor !== document.documentElement) {
		...
		if (style.contain.includes("paint")) {
			...
			if (clientX < ancestorRect.left || clientX > ancestorRect.right || clientY < ancestorRect.top || clientY > ancestorRect.bottom) return false;
		} else if (CLIPPING_OVERFLOW_VALUES.has(style.overflowX) || CLIPPING_OVERFLOW_VALUES.has(style.overflowY)) {
			if (clipsAtPoint(style, ancestor.getBoundingClientRect(), clientX, clientY)) return false;
		}
