...
const OverlayCanvas = (props) => {
	...
	const parseBorderRadii = (borderRadius) => {
		...
		const [topLeft = 0, topRight = topLeft, bottomRight = topLeft, bottomLeft = topRight] = borderRadius.split("/")[0].trim().split(/\s+/).map((value) => parseFloat(value) || 0);
		return [
			topLeft,
			topRight,
			bottomRight,
	...
	const clampRadii = (radii, halfWidth, halfHeight) => radii.map((radius) => Math.min(radius, halfWidth, halfHeight));
	const drawRoundedRectangle = (context, rectX, rectY, rectWidth, rectHeight, cornerRadii, fillColor, strokeColor, opacity = 1) => {
		if (rectWidth <= 0 || rectHeight <= 0) return;
		const clamped = clampRadii(cornerRadii, rectWidth / 2, rectHeight / 2);
		context.globalAlpha = opacity;
		context.beginPath();
		if (clamped.some((radius) => radius > 0)) context.roundRect(rectX, rectY, rectWidth, rectHeight, clamped);
	...
	const renderDragLayer = () => {
		...
		drawRoundedRectangle(context, dragAnimation.current.x, dragAnimation.current.y, dragAnimation.current.width, dragAnimation.current.height, dragAnimation.borderRadii, style.fillColor, style.borderColor);
	};
	const renderSelectionLayer = () => {
		...
		for (const animation of selectionAnimations) {
			...
			drawRoundedRectangle(context, animation.current.x, animation.current.y, animation.current.width, animation.current.height, animation.borderRadii, style.fillColor, style.borderColor, effectiveOpacity);
		}
	};
	const renderBoundsLayer = (layerName, animations) => {
		...
		for (const animation of animations) drawRoundedRectangle(context, animation.current.x, animation.current.y, animation.current.width, animation.current.height, animation.borderRadii, style.fillColor, style.borderColor, animation.opacity);
	};
	const hatchPatternCache = /* @__PURE__ */ new Map();
	const getOrCreateHatchPattern = (context, color) => {
		const cached = hatchPatternCache.get(color);
		if (cached) return cached;
		const patternCanvas = new OffscreenCanvas(10, 5 + 3);
		const patternContext = patternCanvas.getContext("2d");
		if (!patternContext) return null;
		patternContext.clearRect(0, 0, patternCanvas.width, patternCanvas.height);
		patternContext.fillStyle = color;
		patternContext.fillRect(0, 0, 1, 5);
	...
	const appendBoundsToPath = (path, animation) => {
		const { x, y, width, height } = animation.current;
		if (width <= 0 || height <= 0) return;
		const clamped = clampRadii(animation.borderRadii, width / 2, height / 2);
		if (clamped.some((radius) => radius > 0)) path.roundRect(x, y, width, height, clamped);
		else path.rect(x, y, width, height);
	};
	const buildBoundsPath = (animation) => {
	...
	const fillWithHatch = (context, path, color) => {
	...
	const renderInspectLayer = () => {
		...
		if (gapRects && gapRects.length > 0) {
			...
			for (const gapRect of gapRects) if (gapRect.width > 0 && gapRect.height > 0) gapPath.rect(gapRect.x, gapRect.y, gapRect.width, gapRect.height);
	...
	const runAnimationFrame = () => {
		...
		for (const animation of Object.values(boxModelAnimations)) if (animation.isInitialized) {
			if (interpolateBounds(animation, .95)) shouldContinueAnimating = true;
		}
		compositeAllLayers();
	...
	createEffect(on(() => [props.inspectVisible, props.inspectBoxModel], ([isVisible, boxModel]) => {
		...
		for (const layer of [
			...
			if (existing) updateAnimationTarget(existing, bounds);
			else boxModelAnimations[layer] = createAnimatedBounds(`boxmodel-${layer}`, bounds);
		}
		scheduleAnimationFrame();
	}));
	onMount(() => {
