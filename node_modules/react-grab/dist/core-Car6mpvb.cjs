 ...
 */
const require_state = require('./state-Cgc9de9f.cjs');
const require_freeze_updates = require('./freeze-updates-Dwz1Wyi6.cjs');
const require_extract_element_css = require('./extract-element-css-CdJ_In36.cjs');
	...
	return require_state.createRoot((dispose) => {
		...
		require_state.createEffect(require_state.on(isActivated, (activated, previousActivated) => {
			if (activated && !previousActivated) {
				...
				require_freeze_updates.buildPrehitIndex();
				document.body.style.touchAction = "none";
				unlockViewportZoom = lockViewportZoom();
			} else if (!activated && previousActivated) {
				require_freeze_updates.destroyPrehitIndex();
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CLl5bg6z.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
