 ...
 */
const require_state = require('./state-B1b421NP.cjs');
const require_freeze_updates = require('./freeze-updates-V2LEkDlH.cjs');
const require_extract_element_css = require('./extract-element-css-DJP-v4sr.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CE60xNdw.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
