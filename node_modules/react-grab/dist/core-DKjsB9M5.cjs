 ...
 */
const require_state = require('./state-CzUZ2qZW.cjs');
const require_freeze_updates = require('./freeze-updates-BbnJIlov.cjs');
const require_extract_element_css = require('./extract-element-css-0jm0qAGZ.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CQAU8Abd.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
