 ...
 */
const require_state = require('./state-DJWDIlge.cjs');
const require_freeze_updates = require('./freeze-updates-D6nZGXFt.cjs');
const require_extract_element_css = require('./extract-element-css-CHuybKCL.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CoV6RQ9d.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
