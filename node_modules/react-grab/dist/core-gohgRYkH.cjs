 ...
 */
const require_state = require('./state-Ds9WqH7d.cjs');
const require_freeze_updates = require('./freeze-updates-DG8iRvjr.cjs');
const require_extract_element_css = require('./extract-element-css-Dwph9zw0.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Ccyp1nls.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
