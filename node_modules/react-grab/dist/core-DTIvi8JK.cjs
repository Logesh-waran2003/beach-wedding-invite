 ...
 */
const require_state = require('./state-SAp1Wfda.cjs');
const require_freeze_updates = require('./freeze-updates-Da0PhsK-.cjs');
const require_extract_element_css = require('./extract-element-css-Bhmejwgb.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BOSb9ecS.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
