 ...
 */
const require_state = require('./state-BnF_D-IO.cjs');
const require_freeze_updates = require('./freeze-updates-DQ7ffHQn.cjs');
const require_extract_element_css = require('./extract-element-css-yNRHKyoc.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-G9XYtb4I.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
