 ...
 */
const require_state = require('./state-CCmOu-bE.cjs');
const require_freeze_updates = require('./freeze-updates-CfAA64YT.cjs');
const require_extract_element_css = require('./extract-element-css-CKSyALYy.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BfQLXKGS.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
