 ...
 */
const require_state = require('./state-Dw_r57Jx.cjs');
const require_freeze_updates = require('./freeze-updates-C1hKHvAh.cjs');
const require_extract_element_css = require('./extract-element-css-DVqtz5dq.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Dk2HJUXY.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
