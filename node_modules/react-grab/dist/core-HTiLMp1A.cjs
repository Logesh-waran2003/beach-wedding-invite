 ...
 */
const require_state = require('./state-DZ7gY4Rc.cjs');
const require_freeze_updates = require('./freeze-updates-WGEUsNSc.cjs');
const require_extract_element_css = require('./extract-element-css-hdPj_uGH.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DHOyqgm-.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
