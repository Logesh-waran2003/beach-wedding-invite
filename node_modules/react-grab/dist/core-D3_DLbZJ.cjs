 ...
 */
const require_state = require('./state-K-ziWgc3.cjs');
const require_freeze_updates = require('./freeze-updates-DNqEKS0P.cjs');
const require_extract_element_css = require('./extract-element-css-CX9x0HRm.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-JexMrscR.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
