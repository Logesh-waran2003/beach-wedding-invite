 ...
 */
const require_state = require('./state-OmBUHt3U.cjs');
const require_freeze_updates = require('./freeze-updates-Cs9Y2x_c.cjs');
const require_extract_element_css = require('./extract-element-css-DsBPQxG6.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-o-dH36sw.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
