	...
	contextMenuAction: (api) => ({
		...
		onAction: async (context) => {
			await context.performWithFeedback(async () => {
				const combinedHtml = context.elements.map((element) => element.outerHTML.replace(/\s+data-react-grab-[\w-]*="[^"]*"/g, "")).join("\n\n");
...
const init = (rawOptions) => {
	...
	return createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) import("./renderer-EGajt2k4.js").then(({ ReactGrabRenderer }) => {
			if (disposed) return;
