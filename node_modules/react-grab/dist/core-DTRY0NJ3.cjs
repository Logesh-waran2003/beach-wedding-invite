 ...
 */
const require_state = require('./state-8BEmZSPe.cjs');
const require_freeze_updates = require('./freeze-updates-D_vGaDfI.cjs');
const require_extract_element_css = require('./extract-element-css-j312fDSV.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BykJH0-T.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
