...
const buildPrehitIndex = () => {
	...
	const observer = new IntersectionObserver((entries) => {
		...
		for (const entry of entries) {
			...
			validElements.push(indexedElement);
			validRects.push({
		...
		const tree = new Flatbush(validElements.length);
		for (const rect of validRects) tree.add(rect.left, rect.top, rect.right, rect.bottom);
		tree.finish();
		currentIndex = {
			tree,
...
const getElementAtPosition = (clientX, clientY) => {
	...
	if (isPrehitIndexReady()) {
		const result = queryPrehitIndex(clientX, clientY);
		cache = {
			clientX,
			clientY,
			element: result,
