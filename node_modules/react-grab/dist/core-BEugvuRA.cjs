 ...
 */
const require_state = require('./state-Dk2C2ZZD.cjs');
const require_freeze_updates = require('./freeze-updates-DoCneUcv.cjs');
const require_extract_element_css = require('./extract-element-css-BUj8wPum.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CpvyJZ66.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
