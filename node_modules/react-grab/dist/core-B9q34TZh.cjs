 ...
 */
const require_state = require('./state-CNtEoY0f.cjs');
const require_freeze_updates = require('./freeze-updates-BdGoZFfq.cjs');
const require_extract_element_css = require('./extract-element-css-BQfT1ffU.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-eCC3_G1h.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
