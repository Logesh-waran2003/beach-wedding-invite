 ...
 */
const require_state = require('./state-C9vmQXJz.cjs');
const require_freeze_updates = require('./freeze-updates-DnMwX3pp.cjs');
const require_extract_element_css = require('./extract-element-css-B_9xw1JA.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-WqIltP05.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
