 ...
 */
const require_state = require('./state-CBsxAD3I.cjs');
const require_freeze_updates = require('./freeze-updates-IEnWqVsT.cjs');
const require_extract_element_css = require('./extract-element-css-BPlOOED2.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-o0uWwpmy.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
