 ...
 */
const require_state = require('./state-DfkvYL49.cjs');
const require_freeze_updates = require('./freeze-updates-DEEMah9K.cjs');
const require_extract_element_css = require('./extract-element-css-D1gWlmnO.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-tRR6HodG.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
