	...
	const observer = new IntersectionObserver((entries) => {
		...
		for (const entry of entries) {
			...
			if (computedPosition === "fixed") {
				accumulatedFixedElements.push(targetElement);
				continue;
			}
			if (isDecorativeOverlay(targetElement, computedPosition)) continue;
...
const findTopmostFixedElement = (fixedElements, clientX, clientY) => {
	...
	for (const element of fixedElements) {
		...
		if (clientX >= rect.left && clientX <= rect.right && clientY >= rect.top && clientY <= rect.bottom) {
			if (!topmost) topmost = element;
			else if (compareStackingOrder(element, topmost) > 0) topmost = element;
		}
	}
	return topmost;
};
