 ...
 */
const require_state = require('./state-Bzg83E-f.cjs');
const require_freeze_updates = require('./freeze-updates-DKgZd99h.cjs');
const require_extract_element_css = require('./extract-element-css-Dbg6seRC.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-D-A5xLol.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
