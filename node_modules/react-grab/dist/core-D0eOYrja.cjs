 ...
 */
const require_state = require('./state-BpjzL3jx.cjs');
const require_freeze_updates = require('./freeze-updates-Dsyy1nH9.cjs');
const require_extract_element_css = require('./extract-element-css-CFPLAu-L.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-fErXSae6.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
