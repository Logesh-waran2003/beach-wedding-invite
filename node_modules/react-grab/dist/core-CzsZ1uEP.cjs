 ...
 */
const require_state = require('./state-DwljB0t7.cjs');
const require_freeze_updates = require('./freeze-updates-D6lFSmu7.cjs');
const require_extract_element_css = require('./extract-element-css-vZVwOKZk.cjs');
	...
	return require_state.createRoot((dispose) => {
		...
		require_state.createEffect(require_state.on(isActivated, (activated, previousActivated) => {
			if (activated && !previousActivated) {
				require_freeze_updates.freezePseudoStates(store.pointer.x, store.pointer.y);
				require_freeze_updates.freezeGlobalAnimations();
				require_freeze_updates.buildElementAtPointIndex();
				document.body.style.touchAction = "none";
				unlockViewportZoom = lockViewportZoom();
			} else if (!activated && previousActivated) {
				require_freeze_updates.destroyElementAtPointIndex();
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DvEFxlBx.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
