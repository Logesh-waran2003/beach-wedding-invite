 ...
 */
const require_state = require('./state-zk6q2qma.cjs');
const require_freeze_updates = require('./freeze-updates-CwI6lrr1.cjs');
const require_extract_element_css = require('./extract-element-css-BZXcp3qT.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CLlIQqTY.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
