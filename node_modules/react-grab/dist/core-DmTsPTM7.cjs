 ...
 */
const require_state = require('./state-B_dfrusk.cjs');
const require_freeze_updates = require('./freeze-updates-DTEcw9eo.cjs');
const require_extract_element_css = require('./extract-element-css-CGQ-PDjn.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-rZMe1GAI.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
