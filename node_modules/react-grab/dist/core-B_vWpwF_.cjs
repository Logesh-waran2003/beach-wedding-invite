 ...
 */
const require_state = require('./state-DRdwRhwC.cjs');
const require_freeze_updates = require('./freeze-updates-BToqt4TL.cjs');
const require_extract_element_css = require('./extract-element-css-BOhGp4me.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BEextKJ9.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
