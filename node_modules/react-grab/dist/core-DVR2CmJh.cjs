 ...
 */
const require_state = require('./state-qffOo9uJ.cjs');
const require_freeze_updates = require('./freeze-updates-M0wXvQ-A.cjs');
const require_extract_element_css = require('./extract-element-css-CxHqWuwi.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-C48ZlcNe.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
