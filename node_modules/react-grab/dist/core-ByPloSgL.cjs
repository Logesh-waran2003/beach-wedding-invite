 ...
 */
const require_state = require('./state-W6PhNLhj.cjs');
const require_freeze_updates = require('./freeze-updates-C76siRzc.cjs');
const require_extract_element_css = require('./extract-element-css-BSrv-2xQ.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DKM_QyPJ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
