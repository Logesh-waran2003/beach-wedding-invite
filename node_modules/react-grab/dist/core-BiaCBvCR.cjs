 ...
 */
const require_state = require('./state-Ck3jLgVb.cjs');
const require_freeze_updates = require('./freeze-updates-Cnbyp2EY.cjs');
const require_extract_element_css = require('./extract-element-css-BQ5tl8LI.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CYRgk6Ek.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
