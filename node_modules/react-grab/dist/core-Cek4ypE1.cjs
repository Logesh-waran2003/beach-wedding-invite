 ...
 */
const require_state = require('./state-CqlGkrl0.cjs');
const require_freeze_updates = require('./freeze-updates-6sKPao0A.cjs');
const require_extract_element_css = require('./extract-element-css-D39AErEk.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CK6awN_M.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
