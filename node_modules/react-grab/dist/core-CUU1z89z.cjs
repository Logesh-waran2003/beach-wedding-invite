 ...
 */
const require_state = require('./state-ChWe8ao5.cjs');
const require_freeze_updates = require('./freeze-updates-DcMCOutn.cjs');
const require_extract_element_css = require('./extract-element-css-DWaJXNdH.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-wxnq4J53.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
