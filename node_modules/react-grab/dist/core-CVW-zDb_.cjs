 ...
 */
const require_state = require('./state-DFIaLsB8.cjs');
const require_freeze_updates = require('./freeze-updates-DL6LwF3L.cjs');
const require_extract_element_css = require('./extract-element-css-BC7b_lGR.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-D6Jw1VSA.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
