 ...
 */
const require_state = require('./state-B6gbkUEp.cjs');
const require_freeze_updates = require('./freeze-updates-BhrDc5bd.cjs');
...
const OverlayCanvas = (props) => {
	...
	const renderInspectLayer = () => {
		...
		fillWithHatch(context, buildRingPath(margin, border), require_freeze_updates.BOX_MODEL_MARGIN_HATCH_COLOR);
		context.fillStyle = require_freeze_updates.BOX_MODEL_PADDING_FILL_COLOR;
		context.fill(buildRingPath(padding, content), "evenodd");
		const contentPath = buildBoundsPath(content);
		context.fillStyle = require_freeze_updates.BOX_MODEL_CONTENT_FILL_COLOR;
		...
		if (gapRects && gapRects.length > 0) {
			...
			fillWithHatch(context, gapPath, require_freeze_updates.BOX_MODEL_GAP_HATCH_COLOR);
		}
	};
	...
	require_state.createEffect(require_state.on(() => [props.inspectVisible, props.inspectBoxModel], ([isVisible, boxModel]) => {
