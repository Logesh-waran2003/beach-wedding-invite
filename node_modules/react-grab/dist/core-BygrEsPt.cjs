 ...
 */
const require_state = require('./state-BFKkTkBD.cjs');
const require_freeze_updates = require('./freeze-updates-wHToieB5.cjs');
const require_extract_element_css = require('./extract-element-css-CrEuqjv6.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CQ9mF2Fk.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
