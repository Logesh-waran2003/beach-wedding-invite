 ...
 */
const require_state = require('./state-C9ReJUDP.cjs');
const require_freeze_updates = require('./freeze-updates-C69I8fgr.cjs');
const require_extract_element_css = require('./extract-element-css-xzNeXhvV.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-C8P_TJ5I.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
