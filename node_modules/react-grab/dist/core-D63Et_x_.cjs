 ...
 */
const require_state = require('./state-BC9xteq6.cjs');
const require_freeze_updates = require('./freeze-updates-CLNQ2xaq.cjs');
const require_extract_element_css = require('./extract-element-css-CJIbt9sk.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-lz5XzTEb.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
