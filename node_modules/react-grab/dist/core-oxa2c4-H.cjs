 ...
 */
const require_state = require('./state-Bpc9qSbS.cjs');
const require_freeze_updates = require('./freeze-updates-BuViFQfV.cjs');
const require_extract_element_css = require('./extract-element-css-DPwJ7nbn.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Cquj_FV4.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
