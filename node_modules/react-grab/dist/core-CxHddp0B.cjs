 ...
 */
const require_state = require('./state-Bv7chNDk.cjs');
const require_freeze_updates = require('./freeze-updates-Dx2kbWUj.cjs');
const require_extract_element_css = require('./extract-element-css-CeoOlaGg.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-B5MBsDnZ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
