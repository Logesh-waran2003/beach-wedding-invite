 ...
 */
const require_state = require('./state-CXfLUh-l.cjs');
const require_freeze_updates = require('./freeze-updates-Cf7H5c5Q.cjs');
const require_extract_element_css = require('./extract-element-css-CMmCHYst.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DjfSW1OQ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
