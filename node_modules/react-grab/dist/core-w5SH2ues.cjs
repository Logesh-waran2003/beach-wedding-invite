 ...
 */
const require_state = require('./state-CWMgz66g.cjs');
const require_freeze_updates = require('./freeze-updates-B5SygPbv.cjs');
const require_extract_element_css = require('./extract-element-css-C4e53_Fy.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CCw57WCe.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
