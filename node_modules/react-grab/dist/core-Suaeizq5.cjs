 ...
 */
const require_state = require('./state-BpsGefNi.cjs');
const require_freeze_updates = require('./freeze-updates-C0Csf8-o.cjs');
const require_extract_element_css = require('./extract-element-css-BUAXKv9v.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-D-6d8mhi.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
