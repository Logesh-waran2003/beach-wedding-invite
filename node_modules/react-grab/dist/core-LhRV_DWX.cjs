 ...
 */
const require_state = require('./state-fdPZJagy.cjs');
const require_freeze_updates = require('./freeze-updates-CVwo6QTV.cjs');
const require_extract_element_css = require('./extract-element-css-Cg4h8hNP.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DXvXBXeW.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
