 ...
 */
const require_state = require('./state-X62uwTjS.cjs');
const require_freeze_updates = require('./freeze-updates-CUpSIQ-J.cjs');
const require_extract_element_css = require('./extract-element-css-Dqv12mLO.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CSLTDIFj.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
