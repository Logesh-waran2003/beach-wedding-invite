 ...
 */
const require_state = require('./state-cTK0iCbx.cjs');
const require_freeze_updates = require('./freeze-updates-CLZVbOyJ.cjs');
const require_extract_element_css = require('./extract-element-css-X-FU_kTP.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BDqL5NW4.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
