 ...
 */
const require_state = require('./state-Dc20Z6R4.cjs');
const require_freeze_updates = require('./freeze-updates-BgTu2Lo1.cjs');
const require_extract_element_css = require('./extract-element-css-YelvzXl6.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-C5gEjn2u.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
