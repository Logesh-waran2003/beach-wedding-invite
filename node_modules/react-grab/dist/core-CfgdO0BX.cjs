 ...
 */
const require_state = require('./state-HyI4bHlg.cjs');
const require_freeze_updates = require('./freeze-updates-C7_eqUKG.cjs');
const require_extract_element_css = require('./extract-element-css-BFTEKDQe.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-B9PTR241.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
