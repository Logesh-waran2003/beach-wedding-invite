 ...
 */
const require_state = require('./state-Q_K_qg_2.cjs');
const require_freeze_updates = require('./freeze-updates-CrLvoUfU.cjs');
const require_extract_element_css = require('./extract-element-css-COBb3xmF.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DfRi2Z1A.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
