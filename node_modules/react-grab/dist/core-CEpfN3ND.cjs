 ...
 */
const require_state = require('./state-CqaPMOox.cjs');
const require_freeze_updates = require('./freeze-updates-CW3GDMWW.cjs');
const require_extract_element_css = require('./extract-element-css-DGk4T4XP.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DCY4idbh.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
