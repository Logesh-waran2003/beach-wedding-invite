 ...
 */
const require_state = require('./state-D1rKuusb.cjs');
const require_freeze_updates = require('./freeze-updates-BlTLQiSv.cjs');
const require_extract_element_css = require('./extract-element-css-M-eGHD03.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CSfZejoU.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
