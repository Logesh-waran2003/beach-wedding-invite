 ...
 */
const require_state = require('./state-Dfh3KJMp.cjs');
const require_freeze_updates = require('./freeze-updates-CLrto9ig.cjs');
const require_extract_element_css = require('./extract-element-css-Dm6bvVDF.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BgdBMbeQ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
